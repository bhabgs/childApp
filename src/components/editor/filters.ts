export const filter = (data: any, value: string, key: string, name: string) => {
  if (!value) {
    return "--";
  }
  ...
  if (data instanceof Object) {
    return data[value] ? data[value] : "状态未知";
  }
  return "--";
};

export const treeBycodeFilter = (treeData: any, val: any) => {
  let key = "";
  let node = null;
...
export const listFilter = (data: any[], key: string, code: string) => {
...
export const iconObj = {
  ...
  other: "icon-kaifazhezhongxin_kapianzhongxin_qitatubiao",
};
...
export const textFilter = (
  res: {
    ...
    logicMap: any[];
    staticMap: any[];
    thingInst: { thing: { thingPropertyList: any } };
  },
  code: string
) => {
  ...
  return `${map[code] ? (listInfo !== "--" ? listInfo : map[code]) : "--"}${
    filter(thingPropertyList, code, "code", "")
      ? filter(thingPropertyList, code, "code", "")
      : ""
  }`;
};
