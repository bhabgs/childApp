...
// 查询 thingText的租
const getThingTextGroup = (out, name) => {
    if (name)
        return out.find(`.${name}`);
    return [
        ...out.find(`.${groupNames.thingDefTextGroup}`),
        ...out.find(`.${groupNames.thingTextGroup}`),
        ...out.find(`.${groupNames.thingInputGroup}`),
        ...out.find(`.${groupNames.thingSwitchGroup}`),
        ...out.find(`.${groupNames.thingButtonGroup}`),
    ];
};
// 设置复杂thing text 的组主题
...
const change = (group, thingTextInfo, val, themeType) => {
    ...
    if (val) {
        ...
        btnText.setAttrs({ x: btnRect.x() + 5 }, { text: thingTextInfo.switchOpt.checkedLabel });
        ...
        const btnText = group.children.find((ele) => ele.name() === "btnText");
        btnText.setAttrs({ x: checkRect.x() + checkRect.width() + 5 }, { text: thingTextInfo.switchOpt.unCheckedLabel });
    }
};
var switchText = {
    add: (themeType, data, position, group) => {
