...
export class Search {
  constructor(data: dataObj) {
    ...
    this.childFiled = data.childFiled || 'child';
    this.onSearch();
  }
  ...
  public getkeyList(value: string, tree: string | any[], keyList: any[]) {
    ...
    for (let i = 0; i < tree.length; i++) {
      ...
      //如果拥有孩子继续遍历
      if (node[this.childFiled]) {
        this.getkeyList(value, node[this.childFiled], keyList);
      }
    }
  ...
  public getParentKey(key: any, tree: string | any[]) {
    ...
    for (let i = 0; i < tree.length; i++) {
      ...
      if (node[this.childFiled]) {
        ...
        //如果不存在，继续遍历其子节点
        if (node[this.childFiled].some((item: { key: any }) => item.key === key)) {
          parentKey = node.key;
        } else if ((temp = this.getParentKey(key, node[this.childFiled]))) {
          //parentKey = this.getParentKey(key,node[this.childFiled])
          //改进，避免二次遍历
          parentKey = temp;
        }
      }
